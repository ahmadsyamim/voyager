    ...
    public function updateDataType($requestData, $throw = false)
    {
        try {
            ...
            if ($this->fill($requestData)->save()) {
                // If the model name is provided but the class doesn't exist, create it automatically
                if (strlen($this->model_name) != 0 && !class_exists($this->model_name)) {
                    // This runs 'php artisan make:model ModelName' behind the scenes
                    Artisan::call('make:model', [
                        'name' => $this->model_name
                    ]);

                    $absolutePath = app_path(str_replace(['App\\', '\\'], ['', '/'], $this->model_name) . '.php');
